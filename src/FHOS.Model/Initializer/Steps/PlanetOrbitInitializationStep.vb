...
Friend Class PlanetOrbitInitializationStep
    ...
    Private Shared Sub PlacePlanetSection(externalActor As IActor, location As ILocation, sectionName As String)
        Dim actor = ActorTypes.Descriptors(ActorTypes.MakePlanetSection(externalActor.Descriptor.Subtype, sectionName)).CreateActor(location, externalActor.Yokes.Group(YokeTypes.Planet).EntityName)
        location.EntityType = LocationTypes.Planet
        actor.Yokes.Group(YokeTypes.Planet) = externalActor.Yokes.Group(YokeTypes.Planet)
    End Sub
End Class
