...
Friend Class PlanetOrbitInitializationStep
    ...
    Public Overrides Sub DoStep(addStep As Action(Of InitializationStep, Boolean))
        ...
        Dim map = MapTypes.Descriptors(MapTypes.PlanetOrbit).CreateMap($"{planet.Properties.LegacyGroups(GroupTypes.Planet).EntityName} Orbit", planet.Universe)
    ...
    Private Shared Sub PlacePlanetSection(externalActor As IActor, location As ILocation, sectionName As String)
        Dim actor = ActorTypes.Descriptors(ActorTypes.MakePlanetSection(externalActor.Descriptor.Subtype, sectionName)).CreateActor(location, externalActor.Properties.LegacyGroups(GroupTypes.Planet).EntityName)
        location.EntityType = LocationTypes.Planet
        actor.Properties.SetGroup(GroupTypes.Planet, externalActor.Properties.LegacyGroups(GroupTypes.Planet))
    End Sub
End Class
