...
Public Module VerbTypes
    ...
    Friend ReadOnly Descriptors As IReadOnlyDictionary(Of String, VerbTypeDescriptor) =
        ...
        {
            {EnterStarSystem, New VerbTypeDescriptor(
                "Enter Star System",
                ...
                Sub(actor)
                    ...
                    End With
                End Sub)},
            {ApproachPlanetVicinity, New VerbTypeDescriptor("Approach Planet", Function(actor) actor.State.Location.Place?.PlaceType = PlaceTypes.PlanetVicinity,
                Sub(actor)
                    DoTurn(actor)
                    With actor.State.Location.Place
                        SetLocation(actor, RNG.FromEnumerable(.Properties.Map.Locations.Where(Function(x) x.Flags(.Properties.Identifier) AndAlso x.Actor Is Nothing)))
                    End With
                End Sub)},
            {ApproachStarVicinity, New VerbTypeDescriptor("Approach Star", Function(actor) actor.State.Location.Place?.PlaceType = PlaceTypes.StarVicinity,
                Sub(actor)
                    DoTurn(actor)
                    With actor.State.Location.Place
                        SetLocation(actor, RNG.FromEnumerable(.Properties.Map.Locations.Where(Function(x) x.Flags(.Properties.Identifier) AndAlso x.Actor Is Nothing)))
                ...
                End Sub)},
            {RefillOxygen, New VerbTypeDescriptor("Refill Oxygen", Function(actor)
                ...
                End Sub)},
            {Refuel, New VerbTypeDescriptor("Refuel", Function(actor)
                ...
                End Sub)},
            {EnterWormhole, New VerbTypeDescriptor("Enter Wormhole", Function(actor)
                ...
                Sub(actor)
                    ...
                    End With
                End Sub)},
            {EnterOrbit, New VerbTypeDescriptor("Enter Orbit", Function(actor)
            ...
            {KnownPlaces, New VerbTypeDescriptor("Known Places...", Function(actor)
                ...
                End Sub)},
            {DistressSignal, New VerbTypeDescriptor("Signal Distress", Function(actor)
    ...
    AvatarModel.FromActor(actor).Vessel.FuelPercent.Value = 0
            ...
            ($"Price {price} jools!", Hues.Black))
                ...
                End Sub)},
            {Status, New VerbTypeDescriptor("Status", Function(actor) True,
                Sub(actor)
                    Return
                End Sub)},
            {MoveUp, New VerbTypeDescriptor("Move Up", Function(actor)
                                                           Return CanMove(actor)
                                                       End Function,
                Sub(actor)
                    Move(actor, Facing.Up)
                End Sub, visible:=False)},
            {MoveRight, New VerbTypeDescriptor("Move Right", Function(actor)
                                                                 Return CanMove(actor)
                                                             End Function,
                Sub(actor)
                ...
                End Sub, visible:=False)},
            {MoveDown, New VerbTypeDescriptor("Move Down", Function(actor)
                                                               Return CanMove(actor)
                                                           End Function,
                Sub(actor)
                ...
                End Sub, visible:=False)},
            {MoveLeft, New VerbTypeDescriptor("Move Left", Function(actor)
                                                               Return CanMove(actor)
                                                           End Function,
                Sub(actor)
                ...
                End Sub, visible:=False)},
            {SPLORRPedia, New VerbTypeDescriptor("SPLORR!!Pedia", Function(actor)
                                                                      Return True
                                                                  End Function,
                Sub(actor)
                    Return
                End Sub)},
            {Crew, New VerbTypeDescriptor("Crew...", Function(actor)
                                                         Return actor.Family.HasChildren
                                                     End Function,
                Sub(actor)
                    Return
                End Sub)},
            {Vessel, New VerbTypeDescriptor("Vessel...", Function(actor)
